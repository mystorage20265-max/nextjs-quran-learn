...
// Audio segment type: [word_index, start_ms, end_ms]
export type AudioSegment = [number, number, number];

export interface VerseAudioSegments {
    verse_key: string;
    url: string;
...
/**
 * Get audio segments for word-by-word highlighting
 */
export async function getAudioSegments(reciterId: number, chapterId: number): Promise<ChapterAudioData | null> {
    try {
        const url = `https://api.qurancdn.com/api/qdc/audio/reciters/${reciterId}/audio_files?chapter=${chapterId}&segments=true`;
        const response = await fetchWithRetry(url);
        const data = await response.json();
        return data;
    } catch (error) {
...
/**
 * Word type for word-by-word display
 */
...
/**
 * Get verses with word-by-word breakdown
 */
export async function getVersesWithWords(
    ...
    try {
        // Fetch from Quran.com API with words
        const url = `${API_BASE}/verses/by_chapter/${chapterId}?language=en&words=true&word_fields=text_uthmani,text_imlaei&translations=131&per_page=300`;
        const response = await fetchWithRetry(url);
        const data = await response.json();
